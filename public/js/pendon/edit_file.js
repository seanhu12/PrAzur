$(document).ready(function () {
    ...
    $("#custom_file_pendon").change(function () {
        ...
        if (file == null) {
            ...
            if (!(((imagefile == match[0])||(imagefile == match[1])||(imagefile == match[2]))&&fileSize<=5242880)) {
                window.document.getElementById('custom_file_pendon').setAttribute('class', 'form-control is-invalid');
                $(this).siblings(".custom-file-label").html("Seleccionar archivo...");
                ...
                window.document.getElementById('custom_file_pendon').setAttribute('class', 'form-control ');
            }
        }
    });
    ...
    window.enviarArchivo = function (id) {
        ...
        var file = $('#custom_file_pendon').prop('files')[0];
        ...
        $.ajax({
            ...
            success: function(result) {
                // console.log('ajax ok');
                window.location.replace("/pendon/show/"+result);
            },
            error: function(xhr) {
                var error = '';
