$(document).ready(function () {
    ...
    window.enviarArchivos = function (id) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                window.location.replace("/propuesta/show/" + id);
            },
            error: function (xhr) {
    ...
    window.eliminarArchivo = function (id) {
        $.ajax({
            ...
            url: '/propuesta/eliminar_archivo',
            type: 'post',
            data: {
                idArchivo: id
            },
    ...
    var dataAreaPropuesta = "[" + $('#data-tag-area-propuesta').data('data') + "]";
    // Crear elinput-tag (selectize)
    var selectArea = $('#select-beast-area').selectize({
        create: false,
        searchField: 'nombre',
        items: JSON.parse(dataAreaPropuesta),
    ...
    // Empresa de la propuesta
    var dataEmpresaPropuesta = "[" + $('#data-tag-empresa-propuesta').data('data') + "]";
    // Crear elinput-tag (selectize)
    var selectEmpresa = $('#select-beast-empresa').selectize({
        create: false,
        searchField: 'nombre',
        items: JSON.parse(dataEmpresaPropuesta),
    ...
    // Contacto Empresa de la propuesta
    var dataContactoEmpresaPropuesta = "[" + $('#data-tag-contacto-empresa-propuesta').data('data') + "]";
    // Mostrar solo opciones disponibles
    dataContactoEmpresaJson = JSON.parse(dataContactoEmpresaJson);
    var opcionesEmpresa = [];
    dataContactoEmpresaJson.forEach(contactoEmpresa => {
        if (contactoEmpresa.empresa_id.toString() === selectizeControlEmpresa.items[0]) {
            opcionesEmpresa.push(contactoEmpresa);
        }
    });
    // Crear elinput-tag (selectize)
    var selectContactoEmpresa = $('#select-beast-contacto-empresa').selectize({
        create: false,
        searchField: 'nombre',
        items: JSON.parse(dataContactoEmpresaPropuesta),
    ...
    // Contacto Otic de la propuesta
    var dataContactoOticPropuesta = "[" + $('#data-tag-contacto-otic-propuesta').data('data') + "]";
    // Crear elinput-tag (selectize)
    var selectContactoOtic = $('#select-beast-contacto-otic').selectize({
        create: false,
        searchField: 'nombre',
        items: JSON.parse(dataContactoOticPropuesta),
    ...
    var dataProgramaPropuesta = "[" + $('#data-tag-programa-propuesta').data('data') + "]";
    // Crear elinput-tag (selectize)
    var selectPrograma = $('#select-beast-programa').selectize({
        create: false,
        searchField: 'nombre',
        items: JSON.parse(dataProgramaPropuesta),
    ...
    // Curso de la propuesta
    var dataCursoPropuesta = "[" + $('#data-tag-curso-propuesta').data('data') + "]";
    // Crear elinput-tag (selectize)
    var selectCurso = $('#select-beast-curso').selectize({
        create: false,
        searchField: 'nombre',
        items: JSON.parse(dataCursoPropuesta),
    ...
    // Urgencia de la propuesta
    var dataUrgenciaPropuesta = "[" + $('#data-tag-urgencia-propuesta').data('data') + "]";
    // Crear elinput-tag (selectize)
    var selectUrgencia = $('#select-beast-urgencia').selectize({
        create: false,
        searchField: 'nombre',
        items: JSON.parse(dataUrgenciaPropuesta),
    ...
    var dataComplejudadPropuesta = "[" + $('#data-tag-complejidad-propuesta').data('data') + "]";
    // Crear elinput-tag (selectize)
    var selectComplejidad = $('#select-beast-complejidad').selectize({
        create: false,
        searchField: 'nombre',
        items: JSON.parse(dataComplejudadPropuesta),
    ...
    // complejidad de la propuesta
    var dataPerfilPropuesta = "[" + $('#data-tag-perfil-propuesta').data('data') + "]";
    // Crear elinput-tag (selectize)
    var selectPerfil = $('#input-tags-perfil').selectize({
        ...
        labelField: 'nombre',
        searchField: 'nombre',
        items: JSON.parse(dataPerfilPropuesta),
    ...
    // complejidad de la propuesta
    var dataFocoPropuesta = "[" + $('#data-tag-foco-propuesta').data('data') + "]";
    // Crear elinput-tag (selectize)
    var selectFoco = $('#input-tags-foco').selectize({
        ...
        labelField: 'nombre',
        searchField: 'nombre',
        items: JSON.parse(dataFocoPropuesta),
    ...
    if (JSON.parse(dataProgramaPropuesta)[0] != null) {
        habilitarPrograma();
    } else {
        habilitarCurso();
        document.getElementById('opcion_programa').checked = false;
        document.getElementById('opcion_curso').checked = true;
    }

    // if (document.getElementById('foco-observacion').value !== '') {
    //     document.getElementById('foco-observacion').hidden = false;
    // }

    // if (document.getElementById('experiencia-ads-text').value !== '') {
    //     document.getElementById('experiencia-ads-text').removeAttribute('readonly','');
    // }

    // darle formato a los numeros traidos de la bd
    document.getElementById('cant_total_horas').value = formatoNumeroShow((document.getElementById('cant_total_horas').value).replace(',','.'));
    document.getElementById('uf_hora').value = formatoDineroShow(quitarFormatoDinero(document.getElementById('uf_hora').value));
    document.getElementById('monto').value = formatoDineroShow(quitarFormatoDinero(document.getElementById('monto').value));
