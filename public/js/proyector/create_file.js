$(document).ready(function () {
    ...
    $("#custom_file_proyector").change(function () {
        ...
        if (file == null) {
            ...
            if (!(((imagefile == match[0])||(imagefile == match[1])||(imagefile == match[2]))&&fileSize<=5242880)) {
                window.document.getElementById('custom_file_proyector').setAttribute('class', 'form-control is-invalid');
                $(this).siblings(".custom-file-label").html("Seleccionar archivo...");
                ...
                window.document.getElementById('custom_file_proyector').setAttribute('class', 'form-control ');
            }
        }
    });
    ...
    window.enviarArchivo = function (id) {
        ...
        if (!validarNoNulo(document.getElementById('custom_file_proyector').value, 'custom_file_proyector')) {
            noValido = true;
        }
        if (noValido) {
            return;
        }
        var file = $('#custom_file_proyector').prop('files')[0];
        ...
        $.ajax({
            ...
            type: "POST",
            url: "/proyector/store_archivo/" + id,
            data: data_form,
            cache: false,
            contentType: false,
            dataType: 'text',
